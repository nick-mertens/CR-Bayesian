...
df_prob = df %>% group_by(AgeGroup) %>% summarise(prob_pct = sum(q19_2) / n(), count=n())

ggplot(df_prob, aes(x=AgeGroup, y=prob_pct)) + geom_bar(stat="identity") + ggtitle("q19_2 Problem Reporting Rate by Age Group")
```
...
df_make_prob_agg = df[df$MakeName %in% make_list,] %>% group_by(AgeGroup) %>% summarise(prob_pct=sum(q19_2) / n()) 

ggplot(df_make_prob_agg, aes(x=AgeGroup, y=prob_pct)) + geom_bar(stat="identity") +
  ggtitle("q19_2 Problem Reporting Rate by Age Group - Top 10 Make Combined")

for (i in 1:10) {
  make = df_count[1:11,]$MakeName[i]
  df_make_prob_agg = df[df$MakeName == make,] %>% group_by(AgeGroup) %>% summarise(prob_pct=sum(q19_2) / n(), count=n())
  assign(paste("variable", i, sep="_"), ggplot(df_make_prob_agg, aes(x=AgeGroup, y=prob_pct)) + geom_bar(stat="identity") + ggtitle(make) + xlab(NULL) + ylab(NULL) + ylim(0,0.02)) 
}
...
```{r}
make_list = df_count[1:11,]$MakeName
df_make_age = df[df$MakeName %in% make_list,] %>% group_by(AgeGroup, MakeName) %>% summarise(prob_pct=sum(q19_2) / n(), count=n()) 
ggplot(df_make_age, aes(x=AgeGroup, y=MakeName, fill=prob_pct)) + geom_tile() +
...
df_40 <- df_40[df_40$AgeGroup <= 40,] %>% group_by(MakeName) %>% summarise(count_pct = sum(count_pct), count=sum(count), prob_pct=sum(prob_count)/sum(count)) %>% arrange(desc(count_pct))

df_make <- df %>% group_by(MakeName) %>% summarise(avg_prob_pct=sum(q19_2)/n()) 
df_40_make = merge(x=df_40, y=df_make, by="MakeName")
df_40_make[order(-df_40_make$count_pct),]
plot(x=df_40_make$count_pct, y=df_40_make$prob_pct)
text(df_40_make$count_pct, df_40_make$prob_pct, labels=df_40_make$MakeName)
```

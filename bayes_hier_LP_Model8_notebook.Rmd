...
```{r}
df %>% group_by(MakeName) %>% summarise(count=n()) %>% arrange(desc(count))

temp_df = data_filter_func(df, "Nissan")
stan_data = stan_data_func(temp_df, years)

loaded_fit <- readRDS("models/fit_M8_Nissan.rds")
coef = extract_coef_func(loaded_fit)
pred_res = predict_func(stan_data, coef, temp_df)
...
print(temp_df %>% group_by(MMT, MY) %>% summarise(mean_pred_prob=mean(y_pred)))
print(temp_df %>% group_by(MMT, MY) %>% summarise(prob=sum(q19_2) / n()))
```
